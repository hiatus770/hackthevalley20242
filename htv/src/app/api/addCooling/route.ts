...
export async function POST(request: Request) {
    ...
    try {
        const{ 
            user_name, address, contact, model_name, socket_type, fan_size, description, image, rest
        } = await request.json();
        ...
        const query = `
            INSERT INTO COOLING 
            (user_name, address, contact, model_name, socket_type, fan_size, description, image, rest)
            VALUES (${"'" + user_name + "'"}, ${"'" + address+"'"}, ${"'"+contact+"'"}, ${"'"+model_name+"'"}, ${"'"+socket_type+"'"}, ${"'"+fan_size+"'"}, ${"'"+description+"'"}, ${"'"+image+"'"}, ${"'"+rest+"'"}) 
            RETURNING *;
        `;
